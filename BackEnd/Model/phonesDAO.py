...
def getAllPhones():
	try:
		with db.session() as s:
			statment = select(db.Phones).order_by(db.Phones.idPhones)
			r = s.exec(statment)
			return r.all()
	except Exception as e:
		return e
...
def testFilterPhones(data_payload: Any):
	try:
		with db.session() as s:
			
			data_filter = json.loads(data_payload)

			query = select(db.Phones)

			if data_filter:
				for field, value in data_filter.items():
					query = query.filter(getattr(db.Phones, field) == value)
