...
const tabella= async(richiesta,list=[])=> {
	...
	if (richiesta === "People"){
	...
	else if (richiesta === 'Phone'){
		//LISTA DEI CAMPI
		headers = ["","Number", "Prefix","",""];
		...
		for (const phone of data) {
			str = `<tr id="phone${phone.idPhone}">
			<td> <input type="checkbox" id="${phone.idPhone}" name="cancella"></td>
			<td><p id="prefix${phone.idPhone}">${person.name}</p></td>
			<td><p id="number${phone.idPhone}">${person.surname}</p></td>
			<td><button type="button" onclick="modifica('${phone.idPhone}','Phones');">✏️</button></td>
			<td><button type="button" onclick="elimina('${phone.idPhone}','Phones');">🗙</button></td>			
		  </tr>`
		...
		for (const classe of data) {
			...
			<tr id="class${classe.idClass}">
				<td> <input type="checkbox" id="${classe.dClass}" name="cancella"></td>
		...
		for (const vehicle of data) {
			str = `<tr id= vehicle${vehicle.idVehicle}>
			<td> <input type="checkbox" id="${vehicle.idVehicle}" name="cancella"></td>
			<td><p id="Targa${vehicle.idVehicle}">${vehicle.plate}</p></td>
			<td><p id="Tipo${vehicle.idVehicle}">${vehicle.type}</p></td>
			<td><p id="Proprietario${vehicle.idVehicle}">${vehicle.owner}</p></td>
			<td><button type="button" onclick="join('${vehicle.idVehicle}','People','idPeople','Veicoli');">➡️</button></td>
			<td><button type="button" onclick="modifica('${vehicle.idVehicle}','vehicle');">✏️</button></td>
			<td><button type="button" onclick="elimina('${vehicle.idVehicle}','vehicle');">🗙</button></td>	
			</tr>`
		  contentTable += str;
		}
	}
...
const modifica = (id,tipo)=>{
	...
	if (tipo === "People"){
		...
		let oldriga = document.getElementById(`phone${id}`);
		// RECUPERO VALORI DA CONSERVARE
		...
		<tr id="phone${id}">
			...
			<td> <button type="button" onclick="confermaMod('${id}','Phones');">✔️</button></td>
			<td> <button type="button" onclick="elimina('${id}','Phones');">🗙</button></td>			
		</tr>`;
		...
		let oldriga = document.getElementById(`vehicle${id}`);
		// RECUPERO VALORI DA CONSERVARE
		...
		<tr id="person${id}">
			...
			<td> <input type="text" id="proprietarioIn${id}" value="${oldcf}" style=" max-width: 180px"></td>
			<td><button type="button" onclick="join('${person.idPeople}','People','idPeople','Veicoli');">➡️</button></td>
			<td> <button type="button" onclick="confermaMod('${id}','Vehicles');">✔️</button></td>
			<td> <button type="button" onclick="elimina('${id}','People');">🗙</button></td>			
		</tr>`;
