...
const tabella = async(richiesta,list=[])=> {
	...
	if (richiesta.toLowerCase() === "people" || richiesta.toLowerCase() === "person"){
		...
		for (const vehicle of data) {
			...
			<td><button type="button" onclick="modifica('${vehicle.idV}','person');">✏️</button></td>
			<td><button type="button" onclick="elimina('${vehicle.idV}','person');">🗙</button></td>	
			</tr>`
		  contentTable += str;
		}
	}
	...
	<button type="button" class="btn btn-sm btn-primary" onClick="elimina('','${richiesta}');">ELIMINA SELEZIONATI</button>
